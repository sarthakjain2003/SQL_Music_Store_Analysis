...
/* ans:  mohan madan with employee id 9 with level L7*/
...
/* ans: Pargue city with total invoice 273.24 */
...
/* ans: R Madhav having customer id 5 with total invoice 144.54. */
...
/* FIRST FROM CTE (common table expresssion)*/
WITH best_selling_artist AS(
  SELECT artist.artist_id AS artist_id, artist.name AS name, SUM(invoice_line.unit_price*invoice_line.quantity) AS total_sales
  FROM artist
  JOIN album ON artist.artist_id = album.artist_id
  JOIN  track ON album.album_id = track.album_id
  JOIN invoice_line ON track.unit_price = invoice_line.unit_price
  GROUP BY 1        /* artist_id */
  ORDER BY 3 DESC    /* total_sales */
  LIMIT 10
)
...
(
	    ...
	    invoice.billing_country AS country,SUM(invoice.total) AS total_spending,
